...
默认目录是 `/hermes_cron_output/<任务ID>`，文件匹配是 `*.md`。Docker 部署时可将宿主机目录映射为 `/home/ubuntu/.hermes/cron/output:/hermes_cron_output:ro`。登录页面后打开「配置」，可以维护任务源和关注数据规则。
...
- 指标页「刷新」：读取所有已配置来源下尚未入库的匹配文件，增量写入数据库并更新首页展示。
